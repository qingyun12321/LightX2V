    ...
    def _load_ckpt(self, unified_dtype, sensitive_layer):
        ...
        if os.path.isdir(safetensors_path):
            if self.lazy_load:
                ...
                index_path = os.path.join(safetensors_path, "diffusion_pytorch_model.safetensors.index.json")
                if os.path.exists(index_path):
                    with open(index_path, "r") as f:
                        index_data = json.load(f)
                    weight_map = index_data.get("weight_map", {})
                    if weight_map:
                        safetensors_files = sorted({os.path.join(safetensors_path, v) for v in weight_map.values()})
                    else:
